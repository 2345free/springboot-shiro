...
public class MyShiroRealm extends CasRealm {
    ...
    public void initProperty() {
        ...
        this.setCasServerUrlPrefix(ShiroCasConfig.casServerUrlPrefix);
        // 客户端回调地址
        this.setCasService(ShiroCasConfig.shiroServerUrlPrefix + ShiroCasConfig.casFilterUrlPattern);
    }
    ...
    protected AuthorizationInfo doGetAuthorizationInfo(PrincipalCollection principalCollection) {
        ...
        */
         ...
         */
        User user = this.userDao.findByName(loginName);
        if (user != null) {
...
//            }
        ...
        /*
          返回null的话，就会导致任何用户访问被拦截的请求时，
          都会自动跳转到unauthorizedUrl指定的地址
         */
        return null;
    }
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(
        ...
        //查出是否有此用户
        User user = this.userDao.findByName(token.getUsername());
        if (user != null) {
            /*
             若存在，将此用户存放到登录认证info中，
             无需自己做密码对比，Shiro会为我们进行密码对比校验
            */
